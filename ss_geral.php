...
function geraDados ($arr_comp) {
	...
	for ($i=0;$i<count($arr_comp);$i++) {
		...
		*/
		...
		//
		array_push($arr_retorno[$i],substr($arr_comp[$i]["total"],3,8));
		//array_push($arr_retorno[$i],$arr_comp[$i]["total"]);

		//
		array_push($arr_retorno[$i],substr($arr_comp[$i]["dif1"],4,8));
...
$cat_num = $arr_comp[$i]["c13_codigo"];
		...
		*/
		...
		//
		if ($status == 'D') array_push($arr_retorno[$i],"*");
				else array_push($arr_retorno[$i],substr($arr_comp[$i]["total_geral"],1,9));
