...
function geraDados ($arr_comp) {
	...
	for ($i=0;$i<count($arr_comp);$i++) {
		...
		//
		array_push($arr_retorno[$i],substr($arr_comp[$i]["total"],0,8));

		//
		array_push($arr_retorno[$i],round($arr_comp[$i]["velocidade"],2));

		//
		if (substr($arr_comp[$i]["dif1"],0,1)=='-') 	array_push($arr_retorno[$i],substr($arr_comp[$i]["dif1"],1,8));
		else 											array_push($arr_retorno[$i],substr($arr_comp[$i]["dif1"],0,8));
...
if ($cat_txt=="") {
	...
	elseif ($modalidade==5) $cat_txt = "ETVs";
...
$trecho_txt1 = $tre[0]["c02_nome"]." - ".($tre[0]["c02_origem"])." - ".($tre[0]["c02_destino"])." - ".$dist_esp_tot."km";
